...
class Block {
  constructor(index, previousHash, timestamp, data, hash, nonce, randomNum) {
    this.index = index;
...
var MessageType = {
  ...
  var getGenesisBlock = () => {
    return new Block(0, "0", 1682839690, "RUT-MIIT first block",
      "8d9d5a7ff4a78042ea6737bf59c772f8ed27ef3c9b576eac1976c91aaf48d2de", 0);
  };
...
var calculateHashForBlock = (block) => {
  return calculateHash(block.index, block.previousHash, block.timestamp,
    block.data, block.nonce, block.randomNum);
};
var calculateHash = (index, previousHash, timestamp, data, nonce, randomNum) => {
  return CryptoJS.SHA256(index + previousHash + timestamp + data +
    nonce + randomNum).toString();
};
